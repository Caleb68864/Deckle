...
> B22 and the new B38-B40, in the same session, and F1 got its picker half.
> Everything
...
| **N10** | (Unchanged — still open.) **Multi-select rotate/skip/remove** in Arrange. The context menu offers it for N pages; the buttons act on `currentRow()` only. There is no way to *remove* a page, only skip. | | S |
