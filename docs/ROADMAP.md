...
> then fixed on 2026-09-08**, along with B3, B4, B11, B12, B14 and B16, in
> the same session, and F1 got its picker half. Everything
...
| **B25** | `_parse_paper` rejects `cm` while its own error message lists it; disallows the space `_parse_length_pt` allows. | `cli.py:122-132` | S |
...
| **N10** | **Multi-select rotate/skip/remove** in Arrange. The context menu offers it for N pages; the buttons act on `currentRow()` only. There is no way to *remove* a page, only skip. | | S |
