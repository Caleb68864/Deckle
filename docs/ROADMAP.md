...
| **D8** ✅ | CHANGELOG Added: "CLI — impose, export, info, --version" | ~~`crop-preview`, `dummy`, and most export flags appear only under Fixed or not at all~~ **Fixed 2026-09-09.** All six commands and the export flags are in Added; the Fixed entries for `crop-preview` and `dummy` were left alone, being true and dated. **Drifted again and re-fixed 2026-09-11** — this row read "all six commands" while the parser had grown to eight: `print`, `profile`, `--json` and `--dry-run` shipped without reaching the changelog, and the six commands it did name were spelled `deckle <command>`, which is the *GUI* console script. Marking a prose row closed is what let it drift twice, so the second fix is enforced: `test_the_changelog_names_every_cli_command` and `test_the_changelog_spells_a_command_the_way_it_is_installed` walk the real parser, the same way D5's guard does for the GUIDE |
