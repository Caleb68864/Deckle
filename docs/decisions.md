...
- Fix:  names every position by the pages it falls between -- how someone reads a stack, not an index -- and the picker is injectable so the flow is testable headlessly. Blanks are labelled  in the page list.
- Surfaces: Writing those tests surfaced two unrelated defects. First,  had NO error handling: an unreadable source raised inside a QThread, which Qt cannot deliver anywhere, so it printed a traceback and left the grid empty -- the same picture as thumbnails that are merely slow. The realistic cause is mundane, a project reopened after its source PDF moved. Second,  inspected the live , asserting "nothing in this pytest session has loaded Qt" rather than the real invariant. Every Qt-using test file passed it only because their names sort AFTER ; a new file named  broke it while changing nothing about the core.
- Watch: The purity check now runs in a subprocess and walks every core submodule. A first attempt imported only , whose  is nearly empty -- it passed with a deliberate  sitting in models.py. Mutation testing caught that the test was weaker than its own name claimed, which is the only reason it got fixed.
- Commit: (this commit)
