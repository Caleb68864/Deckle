...
- **Acceptance criteria:**
  ...
  - `[MECHANICAL]` `! grep -rn "PIL\|Image.open" deckle/core/layout.py deckle/core/export.py || (echo "FAIL: forbidden pattern present" && exit 1)` — **negative assertion: exits 0 when absent** — image handling exists only in the loader.
...
- **Acceptance criteria:**
  ...
  - `[MECHANICAL]` `! grep -n "add_overlay\|merge_transformed_page" deckle/core/export.py || (echo "FAIL: forbidden pattern present" && exit 1)` — **negative assertion: exits 0 when absent**.
...
- **Acceptance criteria:**
  ...
  - `[MECHANICAL]` `! grep -n "PaperSource" deckle/app/backend.py || (echo "FAIL: forbidden pattern present" && exit 1)` — **negative assertion: exits 0 when absent**.
...
- **Acceptance criteria:**
  ...
  - `[MECHANICAL]` `! grep -n "reverse\|sheet_order\|% 2" deckle/app/views/print_dialog.py || (echo "FAIL: forbidden pattern present" && exit 1)` — **negative assertion: exits 0 when absent** — ordering logic must not leak into the UI.
...
- **Acceptance criteria:**
  ...
  - `[MECHANICAL]` `! grep -rn "PyMuPDF\|import fitz" deckle/ tests/ || (echo "FAIL: forbidden pattern present" && exit 1)` — **negative assertion: exits 0 when absent**.
