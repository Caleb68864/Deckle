...
| 3 | 3 stations, kind `sewing_station`, evenly spaced, first at the tail inset and last at the head inset (REQ-028) | `[BEHAVIORAL]` | `python -m pytest tests/test_marks.py -q -k three_sewing_stations_are_evenly_spaced_between_the_insets` |
| 4 | Every station's x-interval contains `fold_x` (REQ-028) | `[BEHAVIORAL]` | `python -m pytest tests/test_marks.py -q -k every_sewing_station_straddles_the_fold` |
| 5 | `count=0` returns `()`; `count=1` returns one centred station (REQ-028) | `[BEHAVIORAL]` | `python -m pytest tests/test_marks.py -q -k "zero_sewing_stations_returns_no_marks or one_sewing_station_is_centred"` |
| 6 | Order marks step monotonically; index 0 at the tail margin, last at the head margin (REQ-029) | `[BEHAVIORAL]` | `python -m pytest tests/test_marks.py -q -k signature_order_marks_step_monotonically_down_the_spine` |
| 7 | `sig_count == 1` does not divide by zero (REQ-029) | `[BEHAVIORAL]` | `python -m pytest tests/test_marks.py -q -k a_single_signature_does_not_divide_by_zero` |
| 8 | `fold_line` returns one `fold_line` mark with `x0 == x1 == fold_x`, spanning the full sheet height (REQ-030) | `[BEHAVIORAL]` | `python -m pytest tests/test_marks.py -q -k fold_line_is_a_single_segment_on_the_cell_boundary` |
