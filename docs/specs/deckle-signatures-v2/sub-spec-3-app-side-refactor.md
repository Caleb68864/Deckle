...
| 3 | Both leaves of a side can be reported (REQ-013) | [MECHANICAL] | `python -m pytest tests/test_side_reporting.py -q -k both_pages_of_a_side_can_be_reported \|\| (echo "FAIL: clipping loop breaks after the first warning" && exit 1)` |
| 4 | A clean two-page side reports nothing (negative control) | [MECHANICAL] | `python -m pytest tests/test_side_reporting.py -q -k two_page_side_with_no_overflow_reports_nothing \|\| (echo "FAIL: clipping over-reports" && exit 1)` |
...
| 9 | One `Side` is still one PDF page (REQ-012, backend index rule) | [MECHANICAL] | `python -m pytest tests/test_side_reporting.py -q -k back_index \|\| (echo "FAIL: front-then-back index mapping broke under 2-up" && exit 1)` |
