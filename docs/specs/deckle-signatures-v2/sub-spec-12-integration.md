...
| End-to-end folio flow: 8 sheets, 16 PDF pages, two placements per page (REQ-010, REQ-032, REQ-035) | [INTEGRATION] | `python -m pytest tests/test_integration_signatures.py -q -k end_to_end_folio_signature_impose_export_fold_and_print` |
| Fold round trip closes over the exported plan (REQ-019) | [INTEGRATION] | `python -m pytest tests/test_integration_signatures.py -q -k "end_to_end_folio_signature_impose_export_fold_and_print or fold_reading_order_matches_saddle_order_independently_for_the_real_plan"` |
| No orphaned core modules (REQ-021, REQ-031) | [INTEGRATION] | `python -m pytest tests/test_integration_signatures.py -q -k signatures_and_marks_modules_are_invoked_and_marks_consumed_by_export` |
