...
| Signature selector populates `sheets=` (REQ-038) | [MECHANICAL] | `python -m pytest tests/test_print_dialog.py -q -k "selecting_a_signature_passes_its_sheet_indices_to_the_session or selecting_all_passes_sheets_none"` |
| Subset path covers exactly one signature (REQ-035) | [MECHANICAL] | `python -m pytest -q -k plan_passes_over_one_signature_covers_only_that_signature` |
