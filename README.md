...
| **Source cropping** | **Works.** `--crop L,B,R,T` removes space the source already has, with `--crop-even` for a scan whose gutter alternates, and `--auto-crop` to measure it from where the ink actually is. Every other setting adds space; this is the only one that takes it away, and it is what keeps type readable at a small trim size. |
| **Custom signature lengths** | **Works.** `--signatures 10,10,8` states each gathering's sheet count outright, for a page count that divides badly or to land a chapter break on a signature boundary. |
